...
from caiman.tests.comparison import comparisonplots as comparison
import numpy as np
import os
import caiman as cm
...
def test_general():
    ...
    #################### ########################
    ids = comp.save_with_compare(params=params_movie, Cn=Cn)
    nb_view_patches(Yr, A.tocsc()[:, ids[1]],
                                          C[ids[1]], b, f, dims[0], dims[1],
                                          thr=0.8, image_neurons=Cn, denoised_color='red')
    nb_view_patches(Yr, A.tocsc()[:, ids[1]],
                                          C[ids[2]], b, f, dims[0], dims[2],
                                          thr=0.8, image_neurons=Cn, denoised_color='red')
