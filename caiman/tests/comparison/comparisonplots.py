...
class Comparison(object):
    ...
    def save_with_compare(self, params=None, dview=None, Cn=None):
...
\image caiman/tests/comparison/data.pdf
        ...
        try:
            ...
            print("\n")
        info , ids = cnmf(Cn=Cn, A_gt=A_full,
        ...
        return ids,A_full.toarray(),C_full,dims_gt
...
def cnmf(Cn, A_gt, A_test, C_gt, C_test, dims_gt, dims_test, dview=None, sensitivity=0, timer=0, n_frames_per_bin=10):
    ...
    idx_gt, idx_tp, idx_fn, idx_fp, performance_off_on = \
    ...
    corrs = np.array([scipy.stats.pearsonr(
        C_gt_thr[gt, :], C_test_thr[comp, :])[0] for gt, comp in zip(idx_gt, idx_tp)])
    ...
    info = {'isdifferent': int(isdiff),
            'diff_data': {'performance': performance_off_on,
                          ...
                          'diffneur': diffneur},
            'diff_timing': timer}
    return info, [idx_tp, idx_fn, idx_fp]
